from main import MongoDB, User, Bot, GameType, Tournament, Match
...
if __name__ == "__main__":
    ...
    chess_id = game_types.create_game_type(
        "Chess",
        "Classical chess game with standard rules"
    )
    
    user_id1 = users.create_user(
        "standard_user",
        "hashed_password",
        "standard"
    )

    user_id2 = users.create_user(
        "premium_user",
        "hashed_password",
        "premium"
    )
    
    bot_id1 = bots.create_bot(
        "Chess_Bot_1",
        chess_id,
        "chess_bot_code_here"
    )

    bot_id2 = bots.create_bot(
        "Chess_Bot_2",
        chess_id,
        "chess_bot_code_here"
    )
